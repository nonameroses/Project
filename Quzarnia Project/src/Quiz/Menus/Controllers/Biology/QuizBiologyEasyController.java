...
public class QuizBiologyEasyController implements Initializable {
    ...
    List<QuestionBiologyEasy> questionsBiologyEasy;
    ...
    public void gettingTopic(){
        ...
        questionsBiologyEasy = makingQuestions.getQuestionBiologyEasy();
        Collections.shuffle(questionsBiologyEasy);
        labelQuestionNumber.setText("Question 1 of " + questionsBiologyEasy.size());
        loadQuestion(questionsBiologyEasy.get(currentQuestion));

    }

    public void handlerButtonSubmit(ActionEvent actionEvent) {
        currentQuestion++;
        if(currentQuestion < questionsBiologyEasy.size()){
            progressBar();
            labelQuestionNumber.setText("Question " + (currentQuestion + 1 ) + " of " + questionsBiologyEasy.size());
            loadQuestion(questionsBiologyEasy.get(currentQuestion));
        }else{
            Alert alert = new Alert(Alert.AlertType.WARNING);
            alert.setTitle("You finished the quiz!");
            alert.setContentText("Congratulations! Your score is: " + (correctAnswers * 100 / questionsBiologyEasy.size() + "%") );
            alert.showAndWait();
    ...
    public void progressBar(){
        ...
        if(progress < questionsBiologyEasy.size()){
            progress += 0.076;
        }

    }

    public void loadQuestion(QuestionBiologyEasy question){
